...
│   │   ├── NoteAutosave.MD        # next: save-on-first-content, autosave, PUT upsert + DELETE
...
**Backend — two read slices, both range-filtered by calendar date.** `GET /api/schedules/{schedule}/notes?searchFrom=…&searchTo=…` lists a Schedule's notes (`s1`/`s3`/`s6`) whose `Day` falls in the **half-open** window `[searchFrom, searchTo)`, newest first by `(Day, PeriodOrdinal)`, served from an in-memory repository seeded across today ± 3 days. Both bounds are required plain dates (`2026-08-27`), a window wider than 7 days is a `400` (`GetNotesByScheduleValidator`), and a `{schedule}` that is not one the app offers is a `400` naming the parameter. `GET /api/schedules/{schedule}/note-days` answers the same window as `[{ day, count }]` for the calendar's markers — ascending, days with no notes omitted, capped at **42 days** (`GetNoteDaysByScheduleValidator`, the widest month grid). Repository and endpoint tests cover the ranges, the boundaries and every rejection. Nothing else exists: no writes, no Schedule endpoints, no persistence, no auth.

**`note-days` is a plain group-by-day.** There is no offset to reconcile — a note's `Day` is already the caller's calendar day — so grouping is `GroupBy(note => note.Day)` and nothing is converted. Grouping is done in the repository, not the endpoint: it is a query concern, and a real store pushes it into a `GROUP BY`.

**A note's identity is `(ScheduleSpanHours, Day, PeriodOrdinal)`, and there is no id.** A period holds at most one note, so the period *is* the address: `/api/schedules/s3/notes/2026-09-05/p4`. The ordinal is 1-based (`s3`'s 09:00–12:00 is `p4`) and the day is a plain `DateOnly`. `Note` carries no `OccursAt`; an instant let *rendering* and *addressing* disagree, which is how a second, unreachable note in one period became constructible. Every write this key enables is idempotent for free — a `PUT` to a period creates or replaces, and sending it twice is sending it once.

**The server has no timezone.** It stores a date and an integer, and reads a clock only for the two audit stamps. No route takes an offset, and **nothing in the API may gain an `?offset=` query parameter** — that is the plausible helpful addition that would hand the period grid back to the server and reintroduce everything the key removes. The grid is the viewer's local one; the record is a claim about the writer's local calendar.

**A Schedule *is* its span in hours.** `s3` on the wire, `3` in the model. `Models/Schedules.cs` is the one place the backend knows a Schedule's shape — `Known`, `IsKnown`, `PeriodCountFor` (`24 / span`), `IsValidPeriodOrdinal`, and the `TryParse`/`Format` pair that is the only place the `s` sigil is added or removed. `Known` gates on `{1, 3, 6}` rather than on "divides 24", so the server never answers for a grid no client builds; a test asserts every member divides 24 exactly, so adding a span that does not fails the build. Adding `2` or `12` is a one-line edit there.

**`CreatedAt`/`ModifiedAt` are server-zone audit stamps, and nothing renders them.** They record when a note was written, on the *server's* clock, and nothing reads them for placement — placement comes from `(Day, PeriodOrdinal)` and nothing else. If either is ever displayed it must be labelled as server time rather than reinterpreted into the viewer's zone, which would put an instant back into the part of the model the period key clears.

**Frontend — three days at a time, a live clock, and a month calendar to move between them.** The app shows the focus day and its two neighbours as day sections in one scroll container (the page itself never scrolls), fetches through the Vite dev proxy, and opens a period's one note — new or existing — in a `NoteDialog` wrapping `NoteEditor`. The window never grows: three contiguous days is one request, and a navigation toolbar — *Calendar* and *Go to today* — sits above the scroller and never scrolls away. *Calendar* opens a `CalendarDialog`, a Monday-first month grid marked from the `note-days` route, which re-points the view at any day picked. `useNow` ticks on the minute and resyncs on visibility/focus, so the current-period marker moves and an untouched view rolls over at midnight; a view the user has committed to stays put, and a notice says so whenever the focus day is not the current one. Save still only `console.log`s the markdown and the period's address — there is no create/update endpoint. There is no router and no state management library; the selected Schedule, what the user has pinned and the month on show are React state in `App.tsx`. Vitest + React Testing Library are set up and the slice is covered by tests.

**There is also a browser acceptance layer.** 231 Vitest tests cover the units; on top of them sit 63 Playwright specs in the hermetic `stubbed` lane and 4 in the opt-in `integrated` one, covering the journeys and the three things jsdom simulates away — the native `<dialog>`'s modality and its Escape path, real layout (row proportionality, the mount-time scroll into view, a toolbar that stays put), and MDXEditor's typing, toolbar and markdown shortcuts.
...
**Layout inside `TimelyNotes.API`:** `Models/` (entities, plus the `Schedules` shape helper), `Repositories/` (interfaces and implementations side by side), `Endpoints/` (FastEndpoints). Repositories abstract persistence so the store can be swapped later; `InMemoryNoteRepository` is registered as a **singleton** so its seeded state survives across requests.
...
**Layout inside `src/`:** `main.tsx` (root render), `App.tsx` + `App.css` (shell and schedule styles), `types/` (shared `DayKey`/`Note`/`Schedule`/`Period`/`SpanHours`), `domain/` (pure time logic — `schedules.ts`, `periods.ts`, `notes.ts`, `days.ts`, `months.ts`), `hooks/` (`useNow.ts`, `useScheduleNotes.ts`, `useNoteDays.ts`), `api/` (`notesApi.ts`, a `fetch` wrapper returning parsed `Note`s and `NoteDay`s), `components/` (`SchedulePicker`, `ScheduleView`, `DaySection`, `PeriodRow`, `NoteDialog`, `NoteEditor`, `CalendarDialog`, `MonthGrid`), `test/` (Vitest setup and the `inTimezone` helper), `index.css` (minimal global styles; no design system).

**`App` owns the domain calls.** It runs `buildPeriods` then `assignNotes` per rendered day in one `useMemo` and hands `ScheduleView` a finished `DayView[]`; nothing below `App` calls the domain functions or fetches, so component tests pass in fixed periods. The selection is stored as its **address** — `{ day, ordinal }` — not as a `Period` object: periods are rebuilt whenever the notes or the Schedule change, so a held reference goes stale. It also owns the wanted window, which it hands to `useScheduleNotes` as `DayKey`s.

**A day is the string `2026-09-05`, branded as `DayKey`, and the wire form *is* the in-memory form.** Nothing is parsed on the way in and nothing formatted on the way out; equality and ordering are lexicographic, correct because the format is fixed-width ISO, and identical in every timezone — which an epoch-ms key was not, being a *different number* for the same calendar day in a different zone. `toDayKey` is the one way in for a day from outside, and it rejects anything not `YYYY-MM-DD` rather than letting it through. Only two functions in the app turn a `DayKey` into a `Date` — `addDays` in `days.ts` and `weekStartOf` in `months.ts`, both through local clock fields — and each has a test that fails east and west of Greenwich. **Never `new Date(dayKey)`:** `new Date('2026-09-05')` is UTC midnight, which is the whole trap the string key removes. `contiguousRuns` and `chunkRun` still keep a request from spanning a cached gap or exceeding the server's 7-day cap.

**`dayKeyOf(now)` is the single clock-to-calendar conversion in the app.** That is where the device's timezone enters, and the only place it should: `useNow` reads the wall clock, `dayKeyOf` says which calendar day that is, and `ordinalOf(now, spanHours)` says which period of it. Everything downstream is arithmetic on labels.
...
**The viewed day and the current day are separate.** `currentDay` comes from the clock; the day being read is `pinned?.day ?? currentDay`, derived rather than stored. `pinned` is `null` until the user commits to something — selecting a row, or opening a dialog — and while it is null the view *is* the clock: the day and the selection both follow it, so a tab left open overnight does the obvious thing. Once pinned, a rollover moves nothing (an open dialog is fixed to the address it was opened on) and the `role="status"` notice says which day is now current; **Go to today** in the toolbar is `setPinned(null)`. Scrolling is viewport navigation only: it neither pins the view nor moves the selection. The calendar is the other way to move, and choosing from it *is* a commitment.

**Memoise on the day key, never on `now`.** `now` is a fresh `Date` every minute, so anything keyed on it — the window, the periods — would be rebuilt each tick and the fetch effect would refire each tick. `currentDay`/`focusDay` are `DayKey`s and are what everything downstream keys off. A tick must never cause a refetch, and there is a test asserting exactly that.

**A period holds at most one note, and `Period` carries no `Date`.** A `Period` is `{ ordinal, note }`; `assignNotes` is a lookup on `(day, ordinal)` rather than a containment scan, and `formatPeriodLabel`/`formatPeriodStart` derive `09:00 – 12:00` from the ordinal and the span. `PeriodRow` renders that one note as **text** beside the period's own label — never as a second control — and the selected row carries a single `Note` button that opens *the* note for that period, existing or empty. There is no second address inside a row, so a row and its button cannot disagree, which is how two notes in one period used to become constructible.

**Place notes by their period, never by an audit stamp.** `createdAt`/`modifiedAt` are the *server's* clock and are not rendered; test fixtures deliberately set them to a different day so the distinction is asserted rather than incidental. If one is ever shown it must be labelled as server time.
...
**Both window bounds go on the wire as the `DayKey`s they already are.** `notesApi.ts` passes `searchFrom`/`searchTo` through untouched and reads `day` back verbatim — there is no serialisation step and nothing in a request an offset could change. Query strings are still built with `URLSearchParams`. Unit tests assert these URLs **structurally** (parse, compare, match a regex) so they pass in any timezone, and the `EITHER_SIDE_OF_GREENWICH` cases assert the request is *identical* east and west.
...
**The dialog opens on an address, not on a note.** `App.handleOpenNote(day, period)` pins the view and sets `dialogSlot = { day, period }`; the note it edits is `slot.period.note`, which is `null` for an empty period. That snapshot is what fixes the note to the slot it was begun in — a note begun at 23:58 and finished at 00:03 stays on the 23:00 period of the old day — and it is the `key` that remounts `NoteEditor`. `occursAtFor` and the client-stamped instant it produced are gone; see `feature-docs/done/LiveClock.MD` §4.
...
- **A pinned timezone means an E2E spec may assert a literal URL** (`searchFrom=2026-08-24`), unlike the unit tests, which must assert structurally to pass anywhere. `api-contract.spec.ts` re-runs it either side of Greenwich: under `America/Los_Angeles` the frozen instant is still the same calendar day, so the request must be **identical**; under `Pacific/Auckland` it is the next day, so the only thing that may differ is *which* day. A `Date` creeping back into the day path is what breaks those.
