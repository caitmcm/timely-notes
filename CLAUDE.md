...
**Frontend — currently red against the backend above.** `OneNotePerPeriod.MD`'s backend half has landed and its frontend half (checklist sections 5–9) has not, so the UI still sends instant bounds and reads `id`/`occursAt`. That is expected and is the next agent's work; the paragraph below describes the frontend as it stands, not as it agrees with the API.
